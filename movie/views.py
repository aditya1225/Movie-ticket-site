...
def Adm_Home(request):
    total_user = Customer.objects.raw("SELECT COUNT(*) FROM movie_customer")
    total_movie = Movie.objects.raw("SELECT COUNT(*) FROM movie_movie")
    total_booking = Booking.objects.raw("SELECT COUNT(*) FROM movie_booking")
    total_cinema = Cinema.objects.raw("SELECT COUNT(*) FROM cinema")

    d = {'total_user': total_user, 'total_movie': total_movie, 'total_booking': total_booking,
         'total_cinema': total_cinema}
...
def Movie_detail(request,pid):
    ...
    if request.method=="POST":
        t = request.POST['time']
        #z = request.POST.get('name1')
        d = request.POST['date']
        c = request.POST['cinema']
        if request.user:
    ...
    d = {'data':data,'cinema':cinema,'time1':time1}
    return render(request,'movie_detail.html',d)

def All_Cinema(request):

    data = Cinema.objects.all()
    d = {'data': data}
    return render(request,'all_cinema.html',d)

def Edit_Cinema(request,pid):
    if not request.user.is_staff:
        return redirect('login')
    data = Cinema.objects.get(id=pid)
    error = False
    if request.method == 'POST':
        ...
        error = True
    d = {'error': error,'data':data}
    return render(request,'edit_cinema.html',d)

def View_Cinema(request):
    if not request.user.is_staff:
        return redirect('login')
    data = Cinema.objects.all()
    d= {'data': data}
    return render(request,'view_cinema.html',d)
...
def Add_Cinema(request):
    ...
    if request.method == 'POST':
        ...
        error = True
    d = {'error': error}
    return render(request,'add_cinema.html',d)

def delete_cinema(request,pid):
    if not request.user.is_staff:
        return redirect('login')
    data = Cinema.objects.get(id=pid)
    data.delete()
    return redirect('view_cinema')
...
def All_Movies(request,pid):
    ...
    if pid == 2:
        ...
        data = Movie.objects.filter(language=lang)
    if pid == 3:
        language = "Kannada Movies"
        lang = Language.objects.get(name="Kannada")
        data = Movie.objects.filter(language=lang)
...
def delete_bookingadm(request,pid):
    if not request.user.is_authenticated:
        return redirect('login')
